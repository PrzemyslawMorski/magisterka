...
for i in range(1, 11):
    ...
    docker_error_rate = 1 - (docker_num_successes/docker_count)

    podman_count = df_podman.shape[0]
    podman_num_successes = len(df_podman[df_podman['success'] == True].index)
    podman_error_rate = 1 - (podman_num_successes/podman_count)

    singularity_count = df_singularity.shape[0]
    singularity_num_successes = len(
        df_singularity[df_singularity['success'] == True].index)
    singularity_error_rate = 1 - (singularity_num_successes/singularity_count)
